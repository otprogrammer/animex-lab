...
const [isPopOver,setIsPopOver] = useState(false)
  ...
  return (
      ...
      <Modal scrollBehavior="inside" backdrop="transparent" isOpen={true}>
        <ModalContent>
    <Popover
      placement="right-end"
      className="sticky"
      showArrow
      backdrop="opaque"
      classNames={{ backdrop : "overflow-hidden"}}
      onClose={() => setIsPopOver(false)}
      
      offset={10}
    >
      <PopoverTrigger onClick={() => setIsPopOver(true)} className="fixed right-2 bottom-2">
        <Button
          variant="shadow"
      ...
      <PopoverContent className="w-[350px] h-[420px] ">
        {(titleProps) => (
          <div className="py-4 px-1  flex flex-col justify-between rounded-xl w-full h-full mx-auto">
            <div className="space-y-2 overflow-y-scroll  my-3 pr-3">
              {content?.map((msg, idx) => (
                      ...
                      <div className="flex">
                        <div
                          className={`w-full text-small font-semibold ${
                            user?.user_metadata?.username == msg?.user?.username
                              ? "text-red-600"
              ...
              <Input
                ...
                endContent={
                  <Tooltip content="Send message">
                    <Button
                      className="focus:outline-none max-w-[20px] w-full h-[30px] bg-white rounded-full"
                      type="button"
