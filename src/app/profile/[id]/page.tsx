...
console.log(user)
  ...
  async function updateProfile({ username, website, avatar_url, coverimage } : any) {
    try {
      setLoading(true);
      ...
      <div>
        <div className="relative">
          <img
            ...
            src={coverUrl ||""} alt="test"
          />
          {user?.user_metadata.username === username && (

          <div>
            <label
              className=" block absolute top-[5rem] right-3  max-w-fit"
              htmlFor="single"
            >
              <span className="py-2 px-3.5 bg-blue-600 rounded-sm text-gray-200 font-bold cursor-pointer hover:bg-blue-700">
                Edit Cover
              </span>
        ...
        <div className=" absolute mt-[-5rem] flex items-center gap-4">
        <div className="relative z-[1] ml-14 w-fit">
            <img
              src={`${avatar_url}`}
 ...
 {user?.user_metadata.username === username && (
            ...
            <div>
              <label
                className=" block absolute top-[-6px] right-0 max-w-fit text-white"
                htmlFor="avatar"
              >
                {uploading ? (
                  <AiOutlineLoading3Quarters
          ...
          <h1 className="  text-3xl text-neutral-300">{username}</h1>
        </div>
        ...
        <div className=" grid grid-cols-3  lg:max-w-[85%] xl:max-w-[75%] ">
              ...
              <hr className={`hidden lg:block h-0.5 w-full transition-all duration-500 ${activeIndex === 1 ? "ml-[100%]"
                : activeIndex === 2 ? "ml-[200%]" :"ml-0"} bg-white`}/>
          ...
          <div className="text-white flex items-center absolute right-3 top-0 h-full">
            {user?.user_metadata.username === username && (
              <span
                ...
                className="bg-blue-600 py-2 px-3.5 rounded-sm cursor-pointer hover:-translate-y-[1px]"
              >
                Edit Channel
              </span>
            )}
      ...
      <div className="flex flex-col lg:flex-row gap-5 py-6 px-8 mb-12 relative text-gray-50">
        <div style={{filter:"drop-shadow(2px 4px 6px black)"}} className=" py-1 px-2  w-full lg:w-[340px] bg-[#0e0e0e] lg:border-[1px] border-neutral-900">
            ...
            <div className="flex gap-2">
              {navbar.map((item, index) => (
                <div
                  ...
                  onClick={() => setCurrentNav(item.name)}
                  className={`flex py-2.5 px-3.5 flex-col gap-[4px] items-center text-sm cursor-pointer ${
                    currentNav === item.name && "border-b-2 border-blue-500"
                  }`}
                >
                  {item.icon}
                  <h1 className="text-gray-300">{item.name}</h1>
          ...
          <div className="overflow-hidden w-full">
            {activeItem === "WATCHLIST" && (
              <div
                ...
                className="mt-2 grid grid-cols-4 gap-2 p-3"
              >
                {watchList?.map((a:any,i:number) => (
                  <div key={i}>

                    <Card {...a} heading={"Recently Watched"}/>
                    <span onClick={async () => {
    await supabase.rpc("delete_watchlist_id",{
      user_id : "c3a476e3-2a01-4430-b2db-7718a4eb5ac2",
      anime_id : a.id
    })}} className="text-white">REMOVE THIS SHIT</span>
                  </div>
